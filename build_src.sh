...
ORIG_TARBALL="${PACKAGE_NAME}_${TIGERBEETLE_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${TIGERBEETLE_VERSION}"

echo "Creating Debian/Ubuntu source packages for tigerbeetle ${TIGERBEETLE_VERSION}-${BUILD_VERSION}..."
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${TIGERBEETLE_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as tigerbeetle-{VERSION}/ which matches our BUILD_DIR
...
tigerbeetle (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${TIGERBEETLE_VERSION}.
